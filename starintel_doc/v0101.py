...
SPEC_VERSION = "0.10.1"
LEGACY_SPEC_VERSION = "0.9.0"
ACCEPTED_SPEC_VERSIONS = frozenset({LEGACY_SPEC_VERSION, SPEC_VERSION})
ADAPTER_VERSION = 2
ALIASES = {
    "organization",
    "organisation",
...
class UnsupportedVersion(ValidationError):
    def __init__(self, value: Any) -> None:
        accepted = ", ".join(sorted(ACCEPTED_SPEC_VERSIONS))
        super().__init__(
            "unsupported_spec_version",
            f"unsupported spec version: {value!r}; accepted during migration: {accepted}",
        )
...
def schema_path() -> Path:
    ...
    if root:
        return Path(root) / "schemas" / "starintel-doc-v0.10.1.schema.json"
    local = Path.cwd() / "schema" / "starintel-doc-v0.10.1.schema.json"
    if local.is_file():
        return local
    return Path.cwd() / "schemas" / "starintel-doc-v0.10.1.schema.json"
...
def category_for(error: JsonSchemaValidationError) -> str:
    ...
    if error.validator == "enum":
        return "unsupported_spec_version" if path == ["schema_version"] else "invalid_enum"
    if error.validator == "const":
        return "unsupported_spec_version" if path == ["schema_version"] else "invalid_constant"
...
def validate_document(
    ...
    if document.get("schema_version") not in ACCEPTED_SPEC_VERSIONS:
...
class Document:
    ...
    def from_compatible_dict(
        ...
        *,
        ...
        migrated = migrate_compatible_document(value, original_uri=original_uri, schema=schema)
        return cls(roundtrip_document(migrated, schema))
...
def capabilities(schema: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "spec_versions": sorted(ACCEPTED_SPEC_VERSIONS),
        "emits_spec_version": SPEC_VERSION,
        "commands": [
            "validate",
            "normalize",
            "roundtrip",
        ...
        "backwards_compatible_read": True,
    }
